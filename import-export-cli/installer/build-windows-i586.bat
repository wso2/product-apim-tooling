...
go build -gcflags=-trimpath=$GOPATH -asmflags=-trimpath=$GOPATH -ldflags "-X main.importExportCLIVersion=%ImportExportCLI_VERSION% -X 'main.buildDate=%UTC_TIME%'" ..\main.go
...
go run -ldflags "-X main.importExportCLIVersion=%ImportExportCLI_VERSION% -X main.importExportCLIPOS=windows -X main.importExportCLIPArch=i586" release.go
