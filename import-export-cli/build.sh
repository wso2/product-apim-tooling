...
do
    ...
    GOOS=$goos GOARCH=$goarch go build \
     -gcflags=-trimpath=$GOPATH -asmflags=-trimpath=$GOPATH \
     -ldflags "-X 'import-export-cli.ImportExportCLIVersion=$build_version' -X 'github.com/wso2/product-apim-tooling/import-export-cli/cmd/mi.Version=$build_version' -X 'github.com/wso2/product-apim-tooling/import-export-cli/utils.MICmd=mi' -X 'github.com/wso2/product-apim-tooling/import-export-cli/cmd/mi.BuildDate=$(date -u '+%Y-%m-%d %H:%M:%S UTC')'" \
     -o $destination $target
